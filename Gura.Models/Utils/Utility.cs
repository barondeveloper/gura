...
namespace Gura.Models.Utils
{
    public static class Utility
    {
        ...
        {
            //for weeks that start on monday
        ...
        public static DateTime EndOfWeek(DateTime userTodayInUtc, int weekStartsOn)
        {
            var start = StartOfWeek(userTodayInUtc, weekStartsOn);
            return start.AddDays(7).AddSeconds(-1);
        }

        public static DateTime StartOfWeek(DateTime dt, int startOfWeek)
        {
            int diff = (int)dt.DayOfWeek - startOfWeek;
            if (diff < 0)
            {
                diff += 7;
            }
